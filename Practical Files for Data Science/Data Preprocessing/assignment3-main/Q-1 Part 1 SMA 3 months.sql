...
GROUP BY `month`, `B2B`),
sma AS 
(SELECT`month`,`B2B`,TotalSales,AVG(TotalSales) OVER (PARTITION BY `B2B` ORDER BY FIELD(`month`, 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec') ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS SMA
FROM monthly_sales)
